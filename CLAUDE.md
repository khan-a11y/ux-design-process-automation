...
> 전역·작업군 CLAUDE.md(`~/.claude`, `D:\AI_Project`) 규칙 상속됨 — 여기엔 이 프로젝트 고유만. `v0.1 · 제작일 2026-06-18`
...
- **AI 엔진 4백엔드:** `localStorage["uxax_studio_settings_v1"]` 전역 공유. 브라우저AI(webllm·WebGPU) / 로컬 Ollama / 수동복붙(기본·무료) / Anthropic API(유료). 추상화 함수 `runLLM(settings,opts)`. 작업별 모델 라우팅 `UXAX.pickModel`(전사=whisper large-v3-turbo, 한국어 생성=EXAONE 우선/API는 sonnet).
...
- 진행 중 대형 플랜: `~/.claude/plans/transient-skipping-emerson.md` (P1 기준 모델 재설계 라운드, Step 2~6 남음)
